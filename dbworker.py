...
def register_new_user(UserID, UserLastName, UserFirstName, UserSex, UserCountry, UserCity, UserDomain, UserPhoto200):
    ...
    c.execute("INSERT INTO user_ball (user_id, user_choose, math_p, russ, obsh, biol, phys, isto, info, himi, lite, geog, lang_e) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)", new_user_ball_param)
    conn.commit()
...
def get_my_ball(user_id):
    c.execute("SELECT math_p, russ, obsh, biol, phys, isto, info, himi, lite, geog, lang_e FROM user_ball WHERE user_id = %d" % user_id)
    result = c.fetchone()
    i = 0
    myballs = []
    for i in range(11):
